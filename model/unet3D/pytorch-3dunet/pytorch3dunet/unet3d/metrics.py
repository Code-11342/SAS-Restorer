...
class ExpandDiceCoefficient:
    ...
    def __init__(self, epsilon=1e-6,  ignore_index=None, **kwargs):
        self.epsilon = epsilon
        self.ignore_index = ignore_index

    def __call__(self, input, target):
        # Average across channels in order to get the final score
        split_input = split_seg(input, ignore_index=self.ignore_index)
        split_target = split_seg(target, ignore_index=self.ignore_index)
        return torch.mean(compute_per_channel_dice(split_input, split_target, epsilon=self.epsilon))
