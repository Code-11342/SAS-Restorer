...
class StandardPredictor(_AbstractPredictor):
    ...
    def __init__(self, model, output_dir, config, **kwargs):
        ...
        self.pd_function = get_pd_function(config)
    ...
    def _save_results(self, prediction_maps, normalization_masks, output_heads, output_file, dataset):
        ...
        # prediction_maps modify
        prediction_maps = np.stack(prediction_maps)
        prediction_maps = prediction_maps[:, :, :, :, :]
        # normalization_masks modify
        normalization_masks = np.stack(normalization_masks)
        ...
        for prediction_map, normalization_mask, prediction_dataset in zip(prediction_maps, normalization_masks,
                                                                          prediction_datasets):
            print(f"test prediction_map  init shape:{prediction_map.shape} mask shape:{normalization_mask.shape}") 
            prediction_map = prediction_map / normalization_mask
            print(f"test prediction_map  div shape:{prediction_map.shape}") 
            prediction_map = self.pd_function(prediction_map[np.newaxis, :, :, :, :])[0][0]
            print(f"test prediction_map  after pd_function shape:{prediction_map.shape}") 
            ...
            print(f"test prediction_map before save shape: {prediction_map.shape}")
            print(f"\ntest prediction_map final shape:{prediction_map.shape} bincount:{np.bincount(prediction_map.flatten())} sum:{np.sum(prediction_map)}\n")
