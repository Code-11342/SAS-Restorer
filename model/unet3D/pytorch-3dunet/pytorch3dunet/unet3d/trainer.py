...
from pytorch3dunet.unet3d.pd_function import get_pd_function
from . import utils

def split_seg(seg, C):
    # split seg of [N,H,W,L] into [N,C,H,W,L]
    N,H,W,L = seg.shape
    split_seg = torch.zeros(size=[N,C,H,W,L])
    for c_value in range(0,C):
        split_channel = torch.where(seg==c_value, 1, 0)
        split_seg[:, c_value, :, :, :] = split_channel
    return split_seg

logger = get_logger('UNet3DTrainer')


def _create_trainer(config, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders, pd_function):
...
class UNet3DTrainerBuilder:
    ...
    def build(config):
        ...
        pd_function = get_pd_function(config)
        ...
        trainer = _create_trainer(config, model=model, optimizer=optimizer, lr_scheduler=lr_scheduler,
                                  loss_criterion=loss_criterion, eval_criterion=eval_criterion, loaders=loaders, pd_function=pd_function)
...
class UNet3DTrainer:
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion,
                 ...
                 skip_train_validation=False, pd_function=None, **kwargs):
    ...
    def from_checkpoint(cls, resume, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders,
                        tensorboard_formatter=None, sample_plotter=None, pd_function=None, **kwargs):
        ...
        for t in self.loaders['train']:
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                if hasattr(self.model, 'final_activation') and self.model.final_activation is not None:
                    ...
                    if(self.pd_function is not None):
                        output = self.pd_function(output)

                # compute eval criterion
                if not self.skip_train_validation:
                    # print(f"test shape output:{output.shape} target:{target.shape}")
