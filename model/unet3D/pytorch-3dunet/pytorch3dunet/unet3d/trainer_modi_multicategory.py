        ...
        for t in self.loaders['train']:
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                if not self.skip_train_validation:
                    predict_output = torch.argmax(output, dim=1)
                    split_predict_output = split_seg(predict_output, C=output.shape[1])
                    split_target = split_seg(target, C=output.shape[1])
                    # print(f"test shape: output:{output.shape} target:{target.shape} split_predict_output:{split_predict_output.shape} split_target:{split_target.shape}")
                    eval_score = self.eval_criterion(split_predict_output, split_target)
