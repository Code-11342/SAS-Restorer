...
def fill_component_3Dlayer(image_cube,seg_cube,black_thresh=300/2300):
    ...
    for slice_h in range(0, h):
        ...
        # print(f"test slice_h:{slice_h}")
        # print(f"test fill_seg_slice:{np.sum(fill_seg_cube[slice_h, :, :])} vol_res_slice:{np.sum(res_slice)}")
        # print(f"test vol_seg_slice:{np.sum(seg_slice)} vol_largest_slice:{np.sum(largest_slice)}")
...
def cal_dice(pd_label, gt_label):
    ...
    # print(f"test shape cal_gt_label:{cal_gt_label.shape} cal_pd_label:{cal_pd_label.shape}")
    # print(f"test vol_intersec:{np.sum(intersec)} vol_cal_gt_label:{np.sum(cal_gt_label)} vol_cal_pd_label:{np.sum(cal_pd_label)}")
